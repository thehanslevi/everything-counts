...
// App Store screenshots at the exact 6.7" size (1290 x 2796). Poster-styled to
// match the app. Designed marketing screenshots are accepted by App Review;
// panel 2 shows the share extension on purpose, to answer guideline 4.2.
const W = 1290;
const H = 2796;
const YELLOW = "#f2c400";
const INK = "#171106";
...
// A bordered poster card block.
function card(x, y, w, h, extra = "") {
  return `<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${PAPER}" stroke="${INK}" stroke-width="7"/>${extra}`;
}
...
// Caption band + a content SVG string → final PNG.
function panel(caption, content) {
...
const panels = {
  ...
  "05-work": panel("EVERY PIECE\nPOOLS", `
    ...
    <text x="90" y="1750" font-family="Georgia" font-size="52" fill="${INK}">this." Shared discovery, finally.</text>`),
};

for (const [name, svg] of Object.entries(panels)) {
  await sharp(Buffer.from(svg)).png().toFile(`screenshots/${name}.png`);
  console.log(`wrote screenshots/${name}.png`);
}
console.log("done — 5 screenshots at 1290x2796");
